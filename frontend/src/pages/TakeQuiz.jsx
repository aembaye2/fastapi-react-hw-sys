...
const TakeQuiz = () => {
    ...
    async (isAutoSubmit = false) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          navigate(
            `/quiz/${id}/results/${response.data.id || response.data._id}`,
          );
        } else {
  ...
  // Timer effect
  useEffect(() => {
    if (!quizStarted || timeLeft === null || timeLeft <= 0 || submitting)
      return;
  ...
  const handleAnswerSelect = (questionIndex, optionIndex) => {
    setAnswers((prev) => {
      const currentAnswerData = prev[questionIndex] || {
        selected_options: [],
        numeric_answer: null,
      };
      const currentAnswers = Array.isArray(currentAnswerData.selected_options)
        ? currentAnswerData.selected_options
        : [];
      ...
      if (isSelected) {
        ...
        return {
          ...prev,
          [questionIndex]: {
            selected_options: currentAnswers.filter(
              (idx) => idx !== optionIndex,
            ),
            numeric_answer: null,
          },
        ...
        return {
          ...prev,
          [questionIndex]: {
            selected_options: [...currentAnswers, optionIndex],
            numeric_answer: null,
          },
        };
      }
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-sm font-medium text-blue-800 mb-2">
                  Important Instructions & Ethics:
                </h3>
                <div className="text-sm text-blue-700 space-y-2">
                  <p>
                    • <strong>Purpose:</strong> This quiz is designed to assess
                    your current knowledge and help you identify areas for
                    improvement.
                  </p>
                  <p>
                    • <strong>Academic Integrity:</strong> Please answer
                    honestly based on your own knowledge. Using AI tools, search
                    engines, or external help defeats the purpose of
                    self-assessment.
                  </p>
                  <p>
                    • <strong>Learning Opportunity:</strong> View incorrect
                    answers as learning opportunities rather than failures.
                  </p>
                  <p>
                    • <strong>Multiple Answers:</strong> Some questions may have
                    multiple correct answers - select all that apply.
                  </p>
                  <p>
                    • <strong>Navigation:</strong> You can move between
                    questions freely and change your answers before submitting.
                  </p>
                  {quiz.time_limit && (
                    <p>
                      • <strong>Time Management:</strong> You have{" "}
                      {quiz.time_limit} minutes. The quiz will auto-submit when
                      time expires.
                    </p>
                  )}
                  <p>
                    • <strong>Confidence Building:</strong> Real confidence
                    comes from genuine understanding, not from shortcuts.
                  </p>
                </div>
              </div>
            </div>
  ...
  const currentAnswers = Array.isArray(currentAnswerData.selected_options)
    ? currentAnswerData.selected_options
    : [];
  const currentNumericAnswer = currentAnswerData.numeric_answer ?? "";
        ...
        <div className="flex justify-between items-center">
          ...
          {timeLeft !== null && (
            <div className="flex items-center space-x-2">
              <Clock
                className={`h-5 w-5 ${timeLeft <= 300 ? "text-red-500" : "text-gray-500"}`}
              />
              <span
                className={`text-lg font-mono ${timeLeft <= 300 ? "text-red-500 font-bold" : "text-gray-700"}`}
              >
                {formatTime(timeLeft)}
              </span>
            </div>
          ...
          currentQuestionType === "formula" ? (
            <div>
              ...
              <input
                type="number"
                step="any"
                value={currentNumericAnswer}
                onChange={(e) =>
                  handleNumericAnswerChange(
            ...
            <button
              ...
              className={`w-10 h-10 rounded-lg font-medium text-sm transition duration-200 ${
                index === currentQuestionIndex
                  ...
                  : answers[index]?.selected_options?.length > 0 ||
                      (answers[index]?.numeric_answer !== "" &&
                        answers[index]?.numeric_answer !== null &&
                        answers[index]?.numeric_answer !== undefined)
